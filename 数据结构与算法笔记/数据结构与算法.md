  ...
  - **排序**：`Array.Sort(名称,排序类对象)`
    ...
    - **复杂排序**：第二个参数传入比较类对象
      ...
      public class CompareStringNumber : IComparer
      {
          ...
          /// <param name="x"></param>
          /// <param name="y"></param>
          /// <returns>1-需要排序，否则不需要排序</returns>
...
- 判断是否是二叉排序树
  ...
  - 思路：使用LDR遍历二叉排序树即可得到从小到大的数值排列，因此可根据LDR结果是否单调递增来判断
...
- 思路：选定初始节点，之后每次选择连接了新节点的最小边进行添加，直到添加了n-1条边
...
- 实现
  ...
  */
  void MiniSpanTree_Prim(G){
  	int min, i, j, k;
  	int adjvex[MAXVEX];		// adjvex[i]：当前生成树内，到目标顶点i的最近顶点
...
- 思路：按权重将边排列，在避免回路的前提下不断加边，直到增加了n-1条边
...
> 从同一个起点出发并到达指定终点的不同路径，去除到终点的那一步可以转换为树状结构
>
> 则可以用DFS计算叶子数量
