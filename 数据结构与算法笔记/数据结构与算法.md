...
- 分析
  ...
  - 若当前访问的子串中出现了重复字符，则无论之后在这个子串的结尾添加什么字符，新的子串都必定有重复字符

    > *abcb* 出现了重复字符，则 *abcbxxxxxx...xxx* 必定都是不符合要求的子串
    >
    > 因此，当 *abcb* 出现重复字符之后，下一次访问必须将左侧移动到b上一次出现的右侧，即从 *cb* 开始访问
...
- 实现
  ...
  public int LengthOfLongestSubstring(string s) 
  {
      ...
      // i为当前子串起点，j指向当前子串结尾的下一个字符
      int i=0,j=0;
      while(i<s.Length && j<s.Length)
      {
          while(j<s.Length)
          {
              if(history.ContainsKey(s[j]))           // 字符出现过
                  if(history[s[j]]>=i) break;         // 上次出现的位置在当前子串中
                  else history[s[j]] = j;
              else history.Add(s[j],j);
              j++;
          }
          // 计算无重复字符子串 s[i:j-1]的长度
          if(j-i>ret) ret = j-i;
          // 下一个子串从重复字符上次出现的位置+1开始，不然总会包含导致重复的两个字符
...
- **定义**
  ...
  int arr[4] = {1,2,3,4};
  int arr[] = {1,2,3,4};
  // 动态创建常在数组长度以参数传入时使用，需要手动释放 free(arr)
...
- **函数传参**：
  ...
  > 从指向一个数组 **退化为指向一个变量**，因此 **必须同时传入数组长度信息**
  
  - **数组形式**：`func(type arr[],int size)` 
  - 指针形式：`func(type* arr, int size)`
...
- 基础操作
  ...
  - 借助 **迭代器** 的 **数据操作**
    ...
    - 删除：`arr.erase(arr.begin()+start, arr.end()-end)`
      ...
      > 此处表示删除index区间：[ start , arr.size()-1-end )
    >
...
- 递归翻转
  ...
  > 翻转后的头结点必然是原先的尾结点
  >
  > 因此可以通过递归翻转的方式先访问到翻转前的尾结点，再通过递归 **逐层返回反转后的头结点**
  ...
  ListNode* reverseList(ListNode* head) 
  {
      ...
      // 递归获取当前节点之后的部分反转后的头结点
      ListNode* temp = reverseList(head->next);
      // 将当前节点连接到之后部分的翻转结果的尾部
      head->next->next = head;
      head->next = nullptr;
...
- **逐个往前挪**：将头结点之后的节点逐个移动到头部之前
  ...
  public ListNode ReverseList(ListNode head) 
  {
      ...
      ListNode pre = head;		// 上一次挪动的节点，每一轮开始时的头结点
      ListNode cur = head.next;	// 当前要挪动的节点
      
      // 每一轮将head往后移动1个位置
      while(true)
      {
          head.next = cur.next;	// 将cur分离出来
          cur.next = pre;			// 将cur作为头节点
          if(head.next==null) return cur;
          pre = cur;
          cur = head.next;
      }
...
- 变种问题：将第left~第right个节点翻转
  ...
  public ListNode ReverseBetween(ListNode head, int left, int right) {
      ...
      temp.next = Revers(temp.next, right-left+1);
  
      return preHead.next;
  }
  
  // 翻转前num个节点
  ListNode Revers(ListNode head, int num)
  {
      if(head==null || head.next==null || num<=1) return head;
      ListNode pre = head;		// 上一次挪动的节点，每一轮开始时的头结点
      ListNode cur = head.next;	// 当前要挪动的节点
...
- 变种问题：分组翻转
  ...
  public ListNode ReverseKGroup(ListNode head, int k) {
      ...
      int groupNum = n/k;
      Console.WriteLine("共{0}个，每组{1}个，有{2}组",n,k,groupNum);
      ...
      {
          tempPreHead.next = Revers(tempPreHead.next, k);
          // 定位下一次翻转的头结点前驱
          for(int j=0;j<k;j++) tempPreHead = tempPreHead.next;
      }
...
- 问题：每位数字都是按照 **逆序** 的方式存储的，请你将两个数相加，并以相同形式返回一个表示和的链表
...
- 定义：`Queue<type> 名称 = new Queue<type>()`
...
public int MaxDepth(TreeNode root) {
    // 非递归dlr
    Stack<KeyValuePair<TreeNode,int>> st = new Stack<KeyValuePair<TreeNode,int>>();
    int curDeep = 0;
    int ret = 0;
    TreeNode cur = root;
    while(st.Count>0 || cur!=null)
    {
        ...
        if(cur.left!=null)
        {
            if(cur.right!=null) st.Push(new KeyValuePair<TreeNode,int>(cur.right, curDeep));
            cur = cur.left;
        }
...
int dfs(int curDeep, TreeNode root)
{
    if(root==null) return curDeep;
    return Math.Max(dfs(curDeep+1,root.left) , dfs(curDeep+1,root.right));
}
```
...
- 实现
  ...
  public int Dijkstra(int n, int[,] edgeArr, int start, int end)
  {
      ...
      for(int i=0;i<n;i++)
      {
          // 找到当前距离start最近，且尚未找到最短路径的顶点:midNode、minDist
          for(int j=0;j<n;j++)
...
- **变种问题**：使得 **有序数组nums** 中出现次数超过两次的元素 **只出现两次** 
  ...
  public int RemoveDuplicates(int[] nums) {
      ...
      /*
          count：保留的元素个数
          nums[0:count-1]：当前确定保留的部分
          	nums[count-1]：上一个保留的元素，保留的部分
          	nums[count]：可用来保留下一个可保留元素的位置
          nums[i]：当前正在被检查是否需要保留的元素
  
          双指针过程：判断nums[i]是否需要保留
              如果nums[i]!=nums[count-2]，则保留nums[i]不会导致某元素重复出现超过2次，可以保留
      */
      int count = 2;
...
- 流程
  <img src="数据结构与算法.assets/直接插入排序.gif" alt="直接插入排序" style="zoom:50%;" />
...
- 实现
  <img src="数据结构与算法.assets/image-20251015143354025.png" alt="image-20251015143354025" style="zoom: 67%;" />
...
> Knuth-Morris-Pratt，多用于 **字符串匹配问题**

##### 前缀函数pre(i)

- 定义：

  - *s* 的子串 **s[0:i]** 的 **最长的** **相等真前后缀的长度**

  - 与s[i]相同的上一个元素的位置为`pre(i)-1`

    > 公共前缀长度为 pre(i)，则前缀末尾对应的index为 `pre(i)-1`

- 示例：aabaaab
  ![image-20220625091606314](数据结构与算法.assets/image-20220625091606314-16561197694321.png)

- **用途**：当在目标字符串str中寻找模板字符串s时，寻找过程中，如果某个位置的字符不匹配，下次不必从s的开头重新去匹配

  > 比如在 `str=aabaabaaabc` 中寻找 `s=aabaaab`
  >
  > 初始化前缀数组，得到 `pre = {0,1,0,1,2,3,1,2,2,3,0}`
  >
  > 其中 `pre[5]=3` 的含义是 `str[0:5]=aabaab` 的相等真前后缀aab的长度为3
  >
  > ------
  >
  > 当遍历到 `str[5]=b` 时，对比部分为：`str[0:5]=aabaab` 和 `s[0:5]=aabaaa`
  >
  > 此时，发现 `str[5]=b` 与 `s[5]=a` 不匹配
  >
  > 此时可以根据 `pre[5]=3` 得知：`str[0:2] = str[3:5] = aab`
  >
  > 由于已经进行到了i=5的比较，因此在之前的比对过程中已经得到了 `str[0:2] = s[0:2]` 的结论
  >
  > 因此，`str[0:2] = str[3:5] = s[0:2]` ，下一轮直接从 `str[5]=b` 与  `s[2]=b` 开始比较
- **性质**
...
- 示例：通过暂存数组pre确定目标串的每个字符发生比较冲突时，**下次比较开始的位置**
    ...
    public int[] GetNext(int[] pattern)
    {
    	...
    	*/
        ...
        while(i<pattern.Length)
        {
            // 情况1：pattern[0:i-1]的前后公共后缀长度为curPublicLength，加入pattern[i]后，长度+1
            if(pattern[i]==pattern[curPublicLength])
            {
                ++curPublicLength;
                next[i++] = curPublicLength;
                ...
                else
                {
                    ...
                    /*
                    	根据性质：与pattern[i-1]相同的上一个字符（即上一轮公共前缀的结尾）index为 next[i-1]-1
                    	可知：
                    		次长公共前后缀的结尾可能是 next[next[i]-1]-1
                    */
                    // 最长公共前后缀的前缀的结尾字符index
                    int preTailIndex = curPublicLength-1;
                    // 次长公共前后缀长度
                    curPublicLength = next[preTailIndex];
                }
        }
...
- 分析：拆分整数n
  ...
  - 递归
    $$
    x^n=\begin{cases} (\sqrt{x})^{2}& \text{n为偶数}\\ (\sqrt{x})^{2}*x& \text{n为奇数} \end{cases}
    $$
...
- 实现
  ...
  public double quickMul(double x, long N) {
      ...
      while (N > 0) 
      {
          ...
          N /= 2;
          Console.WriteLine("ans={0} x_contribute={1} n={2}",ans, x_contribute, N);
      }
      return ans;
  }
  ```
