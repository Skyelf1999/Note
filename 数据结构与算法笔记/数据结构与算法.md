...
- 创建变量
  ...
  - 普通变量：`StructType x;`
  - 指针：`struct StructType *temp = &结构体实例;`
  - 使用**构造函数**的创建
...
- 求长度：`int len = sizeof(arr)/sizeof(arr[0])-1;`
...
- 函数传参
  ...
  - 使用传统定义/数组指针：`int (*arr)[size]`
    ...
    > 传入二维数组名时，这个参数在传递后会退化为一个数组指针
    >
    > 因此在求数组大小时，必须创建引用以接收参数
