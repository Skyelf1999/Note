...
>
> 一般使用抽象类规定类的成员规范，使用接口规定类需要实现的方法
...
>
> 因为set元素值就是其键值，关系到set元素的排序规则。如果任意改变set元素值，会严重破坏set组织。换句话说，set的iterator是一种const_iterator
...
- **函数传参**：`func(type arr[],int size)` 或 `func(type* arr, int size)`
  ...
  - 解决
    ...
    public:
        /*
            步骤：
                1. 搜索目标值
                2. 删除目标节点，将左右子树合并为搜索子树
                3. 将新搜索子树的父节点与被删除节点的父节点相连
        */
        ...
        TreeNode* deleteNode(TreeNode* root, int key) 
        {
            ...
            TreeNode* cur = root;               // 目标节点
            TreeNode* pre = nullptr;            // 目标之父
            bool isLeft = true;                 // 是左子？
            ...
            if(pre)
                if(cur->val>pre->val) isLeft = false;
                else isLeft = true;
    
            TreeNode* newNode = cur;    // 删除后的替代节点
    
            // 删除目标
            if(cur->right)          // 用 右子树最小值 替代
            {
                // 寻找右子树最小值
                newNode = cur->right;
                if(newNode->left)           // 被删除目标右子有左子树
                {
                    while(newNode->left)
                    {
                        cur = newNode;
                ...
                {
                    newNode->left = pre->left->left;
                    newNode->right = pre->left->right;
                    pre->left = newNode;
                }
                else
                {
                    newNode->left = pre->right->left;
                    newNode->right = pre->right->right;
                    pre->right = newNode;
                }
            }
            else if(cur->left)               // 无右子树，用左子树最大值 替代
            {
                // 寻找左子树最大值
                newNode = cur->left;
                if(newNode->right)           // 左子有右孙
                {
                    while(newNode->right)
                    {
                        cur = newNode;
  ...
  - 解决
    ...
    public:
        ...
        TreeNode* myBuildTree(const vector<int>& preorder, const vector<int>& inorder, int preorder_left, int preorder_right, int inorder_left, int inorder_right) {
            ...
            // 在中序遍历中定位根节点
            int inorder_root = menu[preorder[preorder_left]];
            // 得到左子树中的节点数目
...
- 特点：方向不确定，由于油耗可能有剩余，因此可以绕远路/重复走，难以确定状态转移
...
// cache[i,fuel]：从i出发、剩余油量为fuel时，可到达目标的路径数量
int[,] cache;
